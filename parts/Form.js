...
var InputFieldClass = {
	...
	toggleEdit: function(enter){
		if(enter == false && this.state.edit_mode == false) return false
		...
		if(enter == true){
			...
			if(this.props.onSubmit != null && this.refs.input.value) this.props.onSubmit(this.refs.input.value)
	...
	keyPress: function(e){
		e = e.nativeEvent;
		if(e.keyCode == 13 && this.state.edit_mode){
	...
	render: function(){
		...
		var b = (
			<S innerClassName = {'_intui_input_wrapper'}  style = {{color: this.props.c1, background: this.props.c2}} >
				<span style={{position:'relative','color':'rgba('+255+','+255+','+255+',0.5)'}}>{this.props.prefix}</span>
				<input onKeyPress={this.keyPress} autofocus="false" placeholder = {this.props.hint} className = {'_intui_input'} style = {input_style} ref = "input" type={this.props.type} onChange ={this.change} onBlur = {this.toggleEdit.bind(this,false)} ></input>
			</S>
		)
...
var ManyInputFieldClass = {
	...
	toggleEdit: function(){
		...
		if(this.state.edit_mode == true && this.props.onSubmit != null){
			this.props.onSubmit(this.state.parts)
		}
	...
	render: function(){
		return (
			<I beta = {this.props.beta} vertical slide ease = {this.state.edit_mode && ! this.props.error ? Power4.easeOut : this.props.ease} index_pos = {this.props.error ? 0 : (this.state.edit_mode ? 2 : 1)} onHover={this.toggleEdit} >
...
var ToggleFieldClass = {
	...
	render: function(){
		return (
			<S vertical = {this.props.vertical} center beta = {this.props.beta} c = '_intui_form_toggle_slide' >
